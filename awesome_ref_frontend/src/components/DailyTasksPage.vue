...
const {
  viewingPlan, viewingDate, heatmapData,
  loadToday, loadPlanByDate, loadHeatmap, addTask, updateTask, deleteTask
} = useDailyTasks()
...
const curDate = computed(() => viewingDate.value || todayStr.value)
...
const hasPlan = computed(() => !!viewingPlan.value)
const quote = computed(() => quotes[new Date().getDate() % quotes.length])
const tasks = computed(() => viewingPlan.value?.tasks || [])
...
async function prev() { await loadPlanByDate(shiftD(curDate.value, -1)) }
async function next() { const n = shiftD(curDate.value, 1); if (!isFut(n)) await loadPlanByDate(n) }

const hm = computed(() => {
  const map = {}
  for (const i of heatmapData.value) map[i.date] = i

  // Build 52 weeks ending today, columns = weeks, rows = days (Sun=0..Sat=6)
  const today = new Date()
  const endDow = today.getDay()
  const endDate = new Date(today.getFullYear(), today.getMonth(), today.getDate())
  // Go back to the Sunday of the current week
  const start = new Date(endDate)
  start.setDate(start.getDate() - endDow - 52 * 7 + 1)
...
const recent = computed(() => heatmapData.value.slice().sort((a, b) => b.date.localeCompare(a.date)).slice(0, 20))

async function doAdd() {
  const t = newTitle.value.trim(); if (!t) return
  adding.value = true; await addTask(t); newTitle.value = ''; adding.value = false
  await nextTick(); const el = document.querySelector('.dt-list'); if (el) el.scrollTop = el.scrollHeight
}
const stOrd = ['pending', 'partial', 'done']
async function cycle(t) { await updateTask(t.id, { status: stOrd[(stOrd.indexOf(t.status) + 1) % 3] }) }
function togNote(t) { openNote.value = openNote.value === t.id ? null : (noteVal.value = t.note || '', t.id) }
async function saveN(t) { await updateTask(t.id, { note: noteVal.value }); openNote.value = null }
async function delT(t) { await deleteTask(t.id) }
...
async function saveEd(t) { const v = editVal.value.trim(); if (v && v !== t.title) await updateTask(t.id, { title: v }); editId.value = null }
function cancEd() { editId.value = null }

onMounted(async () => { await Promise.all([loadToday(), loadHeatmap()]) })
</script>
  ...
  <div class="dt-page">
    <div class="dt-card">
      ...
      <div class="dt-hd">
        ...
        <div class="dt-di">
          <span class="dt-dd">{{ curDateFmt }}</span>
          <span class="dt-dw">{{ curWeekday }}</span>
          <button v-if="!isToday" class="dt-bt" @click="loadToday()">今天</button>
        </div>
      ...
      <div class="dt-st">
        ...
        <span class="dt-sp"></span>
        <span class="dt-sv dt-wi"><b>{{ partial }}</b> 进行中</span>
      </div>
        ...
        <TransitionGroup name="dt-a">
          <div v-for="t in tasks" :key="t.id" class="dt-tk" :class="{ 'is-done': t.status === 'done', 'is-partial': t.status === 'partial' }">
            <button class="dt-ck" :class="'s-' + t.status" @click="cycle(t)">
              <svg v-if="t.status === 'pending'" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/></svg>
              <svg v-else-if="t.status === 'partial'" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/><path d="M12 3a9 9 0 0 1 0 18" fill="currentColor" opacity=".25"/></svg>
              <svg v-else width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/><polyline points="8 12 11 15 16 9"/></svg>
            </button>
            <div class="dt-tb">
              <template v-if="editId === t.id">
                <input v-model="editVal" class="dt-ed" @keydown.enter="saveEd(t)" @keydown.escape="cancEd" @blur="saveEd(t)" />
              </template>
              <span v-else class="dt-tx" :class="{ done: t.status === 'done' }" @dblclick="startEd(t)">{{ t.title }}</span>
      ...
      <div v-if="!hasPlan || tasks.length === 0" class="dt-em">
        ...
        <p>{{ !hasPlan ? '这天没有计划记录' : isToday ? '还没有任务，写下一个小目标开始吧' : '这天没有任务记录' }}</p>
      </div>
          ...
          <div class="gh-body">
            <template v-for="(dayLabel, row) in ['日','一','二','三','四','五','六']" :key="row">
              ...
              <span v-for="(week, col) in hm.weeks" :key="col"
                ...
                @click="!isFut(week[row].date) && loadPlanByDate(week[row].date)"></span>
            </template>
          </div>
          <!-- Legend -->
          <div class="gh-legend">
      ...
      <div class="dt-rc-sec">
        ...
        <div class="dt-rc">
          <div v-for="i in recent" :key="i.date" class="dt-ri" :class="{ active: i.date === curDate }" @click="!isFut(i.date) && loadPlanByDate(i.date)">
